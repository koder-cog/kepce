use crate::dto::system::{
    ComponentHistoryDto, SystemHealthResponseDto, SystemStatusDto, VerifyTreeResponseDto,
};
...
) -> Result<Json<SystemHealthResponseDto>, AppError> {
    let result = SystemService::get_system_health(&db)
        .await
...
async fn system_status(
    ...
    let result = SystemService::get_system_status(&db)
        .await
        .map_err(|e| AppError::Internal(e.to_string()))?;
    Ok(Json(result))
}
...
async fn verify_tree(
    ...
    require_admin(&user)?;
    let result = SystemService::verify_chain(&db)
        .await
        .map_err(|e| AppError::Internal(e.to_string()))?;
    Ok(Json(result))
}
...
async fn get_status_history(
    ...
    let result = SystemService::get_status_history(&db)
        .await
        .map_err(|e| AppError::Internal(e.to_string()))?;
    Ok(Json(result))
}
