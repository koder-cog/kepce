...
use crate::dto::statistics::{
    ContributorDto, HumanityStatsDto, ModerationCategorySliceDto, ModerationStatsDto,
    RecentActionDto, TopDishDto, TrendingTagDto,
};
use sea_orm::*;
use shared::entities::{dish_tags, prelude::*, reports, sea_orm_active_enums::ReportStatusEnum};
...
impl StatisticsService {
    ...
    pub async fn get_dish_leaderboard(
        ...
        if let Some(slug) = city_slug {
            join_clause =
                "JOIN menus m ON dv.menu_id = m.id JOIN cities ci ON m.city_id = ci.id".to_string();
        ...
        let query = Statement::from_sql_and_values(db.get_database_backend(), &sql, values);
    ...
    pub async fn get_moderation_stats(
        ...
        let status_query =
            Statement::from_sql_and_values(db.get_database_backend(), &status_sql, vec![]);
        let status_rows = db
            .query_all(status_query)
            .await
            .map_err(StatsError::DatabaseError)?;
        ...
        let comments_filter = if !interval.is_empty() {
            format!(
                "WHERE is_deleted = true AND updated_at >= NOW() - INTERVAL '{}'",
                interval
            )
        } else {
            "WHERE is_deleted = true".to_string()
        };
        let del_comments_sql = format!(
            "SELECT CAST(COUNT(id) AS BIGINT) as count FROM comments {}",
            comments_filter
        );
        let del_query =
            Statement::from_sql_and_values(db.get_database_backend(), &del_comments_sql, vec![]);
        let del_row = db
            .query_one(del_query)
            .await
            .map_err(StatsError::DatabaseError)?;
        let deleted_comments: i64 = del_row
        ...
        for row in cat_rows {
            let cat_name: String = row
                .try_get("", "category")
                .unwrap_or_else(|_| "Diğer".to_string());
            let count: i64 = row.try_get("", "count").unwrap_or(0);
        ...
        let act_query =
            Statement::from_sql_and_values(db.get_database_backend(), actions_sql, vec![]);
        let act_rows = db
            .query_all(act_query)
            .await
            .map_err(StatsError::DatabaseError)?;
        ...
        for row in act_rows {
            let action: String = row
                .try_get("", "action")
                .unwrap_or_else(|_| "Kural ihlali işlemi yapıldı".to_string());
    ...
    pub async fn get_trending_tags(
        db: &DatabaseConnection,
        limit: u64,
    ) -> Result<Vec<TrendingTagDto>, StatsError> {
        ...
        let results = TrendingTagResult::find_by_statement(query)
            .all(db)
            .await
            .map_err(StatsError::DatabaseError)?;
    ...
    pub async fn get_humanity_stats(
        db: &DatabaseConnection,
    ) -> Result<HumanityStatsDto, StatsError> {
        ...
        let query = Statement::from_sql_and_values(db.get_database_backend(), sql, vec![]);
