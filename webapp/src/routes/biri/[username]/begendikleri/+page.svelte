...
{:else}
  {#if !dashboardStats?.favorite_comments?.length}
    <EmptyState
      iconName="voteUpFilled"
      title="Henüz Beğeni Yok"
      desc="Başkalarının yorumlarını beğendiğinde burada görünecek."
    />
  {:else}
            ...
            <div class="comment-card__avatar-col">
              <a href="/biri/{c.user?.nickname}" class="comment-card__avatar" data-link>
                {#if c.user?.avatar_url}
                  <img src={api.getAvatarUrl(c.user.avatar_url)} alt="" onerror={(e) => (e.target.outerHTML = icon("avatarEmpty", 32))} />
                {:else}
              ...
              <div class="comment-card__header">
                <div class="comment-card__meta">
                  <span class="comment-card__action-text">
                    <a href="/biri/{c.user?.nickname}" data-link><strong>@{c.user?.nickname}</strong></a>
                  </span>
                  {#if c.is_tabldot || (c.tags && c.tags.length > 0) || (c.tag_ids && c.tag_ids.length > 0)}
                    <span class="comment-node__badge--structured" data-tooltip="Tabldot / Yapılandırılmış yorum">{@html icon("puzzle", 12)}</span>
                  {/if}
