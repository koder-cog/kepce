<script>
  ...
  $effect(() => {
    if (username) {
      ...
      error = {
        status: 404,
        message:
          "Kullanıcı Bulunamadı. Hangi şefin rozetlerine bakmak istiyorsun?",
      };
    }
  });
  ...
  let strokeDashOffset = $derived(
    414.69 * (1 - totalUnlocked / Math.max(totalBadges, 1)),
  );
</script>

<Seo
  title={profile
    ? `@${safeNickname} Rozetleri - Kepçe`
    : "Kullanıcı Rozetleri - Kepçe"}
  description={profile
...
{:else if error}
  <div class="empty-state-container">
    <EmptyState
      ...
      title={error.status === 404 ? "Kullanıcı Bulunamadı" : "Bir Hata Oluştu"}
      desc={error.message}
    >
      {#if error.status !== 404}
        <button
          class="btn btn--primary btn--squish"
          onclick={() => loadProfile(username)}>Tekrar dene</button
        >
      {/if}
    </EmptyState>
  </div>
      ...
      <div class="achievements-header__avatar">
        ...
        <img
          ...
          onerror={(e) => {
            ...
            e.target.src =
              "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22120%22 height=%22120%22 viewBox=%220 0 120 120%22%3E%3Crect width=%22120%22 height=%22120%22 fill=%22%23222%22/%3E%3C/svg%3E";
          }}
        />
      </div>
