<script>
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      showToast(
        name ? `Tekrar hoş geldin, @${name}!` : "Giriş yapıldı!",
        "success",
      );
      close();
    } catch (err) {
...
>
  {#snippet children()}
    ...
    <form
      id="auth-gate-form"
      class="c-modal__form-group u-mt-md"
      onsubmit={handleLogin}
    >
      {#if errorMsg}
        <div class="auth-error" id="auth-gate-error" role="alert">
          {errorMsg}
        </div>
      {/if}
      ...
      {#if isPasswordlessSuccess}
        <div class="passwordless-success u-mb-md">
          ...
          <p class="passwordless-success__desc">
            Eğer <strong>{identifier}</strong> sistemde kayıtlıysa, giriş bağlantını
            e-postana gönderdik.
          </p>
      ...
      {:else}
        <div
          ...
          class:form-group--error={!!errorMsg}
        >
          <input
            type="text"
          ...
          />
          <label class="form-label" for="auth-gate-identifier"
            >{isPasswordlessMode
              ? "Kayıtlı e-posta adresiniz"
              : "E-posta veya kullanıcı adı"}</label
          >
        </div>
