    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        if (modalContainer) {
            ...
            const firstTextarea = modalContainer.querySelector('textarea');
            const lastBtn       = modalContainer.querySelector('.c-modal__footer .btn:last-child');
            (firstInput || firstSelect || firstTextarea || lastBtn)?.focus();
...
<div class="c-modal c-modal--{options.variant || 'standard'} {isOpen && !isClosing ? 'c-modal--open' : ''}" role="dialog" aria-modal="true" bind:this={modalContainer}>
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="c-modal__backdrop" onclick={() => !options.disableEscape && close()}></div>
    
    <div class="c-modal__surface">
        <div class="c-modal__header">
            <h2 class="c-modal__title">{options.title}</h2>
            {#if options.iconHtml}
                <div class="c-modal__icon {options.iconColor === 'danger' || options.iconColor === 'negative' ? 'c-modal__icon--danger' : ''}">
        ...
        <div class="c-modal__content">
            ...
            {#if options.variant === 'lightbox'}
                <button class="c-lightbox-close" aria-label="Kapat" onclick={close}>
                    {@html icon('close', 20)}
                </button>
            {/if}
        </div>
        ...
        {:else if options.buttons && options.buttons.length > 0}
            <div class="c-modal__footer">
                {#each options.buttons as btnConfig}
                    <button class="btn btn--{btnConfig.variant || 'secondary'}" onclick={async (e) => {
                        e.preventDefault();
                        let preventClose = false;
                        if (btnConfig.onClick) {
                            const result = await btnConfig.onClick(modalContainer); 
