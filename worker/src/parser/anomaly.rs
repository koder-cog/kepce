    ...
    MODEL
        .get_or_init(|| {
            match TextEmbedding::try_new(InitOptions::new(EmbeddingModel::ParaphraseMLMiniLML12V2))
            {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Text embedding model yüklenemedi (çevrimdışı fallback aktif): {:?}",
                        e
                    );
                    None
                }
...
mod tests {
    ...
    fn test_lexical_fallback_normal_menu() {
        ...
        assert!(
            dist < 0.40,
            "Geçerli yemek menüsü için mesafe düşük olmalı: {}",
            dist
        );
    }
    ...
    fn test_lexical_fallback_garbage_text() {
        ...
        assert!(
            dist > 0.65,
            "Çöp/hata metninde anomali mesafesi yüksek olmalı: {}",
            dist
        );
    }
